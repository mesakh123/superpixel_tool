...
from PyQt5.QtGui import QIcon, QImage, QPainter, QPalette, QPixmap,QKeySequence
import cv2
import numpy as np
import glob
import copy
...
class Command(QUndoCommand):
    def __init__(self, label,result,paths):
    ...
    def redo(self):
        height, width, channels = np.shape(self.result)
        totalBytes = self.result.nbytes
        bytesPerLine = int(totalBytes / height)
        qimg = QtGui.QImage(self.result.data, self.result.shape[1], self.result.shape[0], bytesPerLine, QtGui.QImage.Format_RGB888)
        pixmap = QtGui.QPixmap.fromImage(qimg)
        self.label.resize(width, height)
        self.label.setPixmap(pixmap)
        self.label.setWindowTitle(self.paths)
        self.label.show()
...
class MyWindow(QMainWindow, form_class):
    ...
    def save_clicked(self):
        if self.image is not None:
            if self.save_path is None or self.save_path is "":
            ...
            if self.save_path:
                ...
                file_name = self.paths[self.index].split('\\')[-1].split(".")
                file_name = file_name[0]+"_mask."+file_name[1]
                cv2.imwrite(self.save_path + '\\' + file_name, save_image)
                QMessageBox.about(self,"Notice","Image saved : "+self.save_path + '\\' + file_name)
            else:
                QMessageBox.about(self,"Notice","Save image canceled")
    ...
    def draw_image(self):
        ...
        self.label.resize(width, height)
        self.label.setPixmap(pixmap)
        self.label.setWindowTitle(self.paths[self.index].split('\\')[-1])
        self.label.show()
        ...
        #add = Command(self.label,result,self.paths[self.index].split('\\')[-1])
